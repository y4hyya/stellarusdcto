...
describe('translateError', () => {
    ...
    test('recognizes wallet rejections from viem, Freighter, and plain messages', () => {
        ...
        expect(translateError(new Error('Transaction rejected by user'), { family: 'solana' }).code).toBe(
            'USER_REJECTED',
        );
    });
    ...
    test('maps the Soroban allowance failure on a burn to the approval message', () => {
        ...
        expect(translateError(raw, { family: 'stellar', phase: 'burning' }).code).toBe('ALLOWANCE_EXPIRED');
    });

    test('maps a missing Stellar account', () => {
        expect(translateError(new Error('Account not found: GDOES...NOTEXIST'), { family: 'stellar' }).code).toBe(
            'ACCOUNT_NOT_FOUND',
        );
    });

    test('recognizes an already used nonce on all three families', () => {
        expect(translateError(new Error('execution reverted: Nonce already used'), { family: 'evm' }).code).toBe(
            'ALREADY_MINTED',
        );
        expect(
        ...
        expect(
            translateError(new Error('Transaction failed: {"InstructionError":[3,{"Custom":6023}]}'), {
                family: 'solana',
            }).code,
        ).toBe('ALREADY_MINTED');
    });
    ...
    test('maps rate limiting, unreachable endpoints, and the max fee floor', () => {
        expect(translateError(new Error('Iris 429: too many requests'), {}).code).toBe('IRIS_RATE_LIMITED');
